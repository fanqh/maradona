...
int UART_IO_Read(UART_IO_HandleTypeDef* h, char* buffer, size_t buffer_size)
{
	...
	status = __CALL UART_IO_RxFlipBuffer(h->handle, (uint8_t*)h->rx_upper, UART_IO_BUFFER_SIZE, &m0ar, &ndtr);
...
n/a		HAL_UART_STATE_ERROR             = 0x04     /*!< Error                                              */  
	...
	if (h->handle->State == HAL_UART_STATE_TIMEOUT ||
			h->handle->State == HAL_UART_STATE_ERROR ||
			h->handle->State == HAL_UART_STATE_RESET)
	{
		errno = EIO;
		return -1;
	}
	...
	while (h->tx_tail < tx_end) {
		*h->tx_tail++ = *buffer++;
		...
		if (count == size) {
			...
			status = __CALL HAL_UART_Transmit_DMA(h->handle, (uint8_t*)h->tx_head, h->tx_tail - h->tx_head);
			if (status == HAL_OK) {
